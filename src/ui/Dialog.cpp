...
void CDialog::build() {
    auto pal = m_backend->getPalette();

    m_window = CWindowBuilder::begin()
                   ->preferredSize({480, 380})
                   ->minSize({480, 280})
                   ->maxSize({560, 600})
    ...
    // Outer column layout, 90% width centered
    auto outer = CColumnLayoutBuilder::begin()->size({CDynamicSize::HT_SIZE_PERCENT, CDynamicSize::HT_SIZE_PERCENT, {0.9F, 1.F}})->commence();
    outer->setMargin(12);
    ...
    // Action icon
    auto iconName = m_req.iconName.empty() ? "system-lock-screen" : m_req.iconName.c_str();
    auto iconDesc = m_backend->systemIcons()->lookupIcon(iconName);
    if (!iconDesc || !iconDesc->exists())
        iconDesc = m_backend->systemIcons()->lookupIcon("dialog-password");
    if (iconDesc && iconDesc->exists()) {
        auto iconWrap = CRowLayoutBuilder::begin()->commence();
        iconWrap->setPositionMode(IElement::HT_POSITION_ABSOLUTE);
        iconWrap->setPositionFlag(IElement::HT_POSITION_FLAG_HCENTER, true);
        iconWrap->addChild(CImageBuilder::begin()->icon(iconDesc)->size({CDynamicSize::HT_SIZE_ABSOLUTE, CDynamicSize::HT_SIZE_ABSOLUTE, {48, 48}})->commence());
        outer->addChild(iconWrap);
    }

    // Title
    {
        auto titleWrap = CRowLayoutBuilder::begin()->commence();
        titleWrap->setPositionMode(IElement::HT_POSITION_ABSOLUTE);
        titleWrap->setPositionFlag(IElement::HT_POSITION_FLAG_HCENTER, true);
        titleWrap->addChild(CTextBuilder::begin()
                                ...
                                ->commence());
        outer->addChild(titleWrap);
    }

    // Subtitle: "for <user>" or identity selector
    {
        auto subWrap = CRowLayoutBuilder::begin()->commence();
        subWrap->setPositionMode(IElement::HT_POSITION_ABSOLUTE);
        subWrap->setPositionFlag(IElement::HT_POSITION_FLAG_HCENTER, true);
        ...
        if (m_req.identities.size() > 1) {
            ...
            subWrap->addChild(cb->commence());
        } else {
            const std::string display = m_req.identities.empty() ? "" : m_req.identities[0].display;
            subWrap->addChild(CTextBuilder::begin()
                                  ->text(std::string{"for "} + display)
                                  ->color([] { return g_pAgent->backend()->getPalette()->m_colors.text.darken(0.3); })
                                  ->commence());
        }
        outer->addChild(subWrap);
    }

    // Message + optional command box
    {
        std::string msg = m_req.message;
        if (!m_req.command.empty())
            msg = "Authentication is needed to run";
        auto msgWrap = CRowLayoutBuilder::begin()->commence();
        msgWrap->setPositionMode(IElement::HT_POSITION_ABSOLUTE);
        msgWrap->setPositionFlag(IElement::HT_POSITION_FLAG_HCENTER, true);
        msgWrap->addChild(CTextBuilder::begin()->text(std::string{msg})->color([] { return g_pAgent->backend()->getPalette()->m_colors.text; })->commence());
        outer->addChild(msgWrap);

        if (!m_req.command.empty()) {
            auto cmdWrap = CRowLayoutBuilder::begin()->commence();
            cmdWrap->setPositionMode(IElement::HT_POSITION_ABSOLUTE);
            cmdWrap->setPositionFlag(IElement::HT_POSITION_FLAG_HCENTER, true);
            auto cmdBg = CRectangleBuilder::begin()
                             ->color([] { return g_pAgent->backend()->getPalette()->m_colors.base; })
                             ->size({CDynamicSize::HT_SIZE_AUTO, CDynamicSize::HT_SIZE_AUTO, {0, 0}})
                             ->rounding(6)
                             ->commence();
            cmdWrap->addChild(cmdBg);
            cmdBg->addChild(CTextBuilder::begin()
                                ->text(std::string{m_req.command})
                                ->fontFamily(std::string{"monospace"})
                                ->color([] { return g_pAgent->backend()->getPalette()->m_colors.text; })
                                ->commence());
            outer->addChild(cmdWrap);
        }
    }

    // Password row: textbox + Show button
    {
        ...
        m_passwordField = CTextboxBuilder::begin()
                              ...
                              ->password(true)
                              ->size({CDynamicSize::HT_SIZE_ABSOLUTE, CDynamicSize::HT_SIZE_ABSOLUTE, {300, 32}})
        ...
        m_revealButton = CButtonBuilder::begin()
                             ...
                             ->onMainClick([this](CSharedPointer<CButtonElement>) {
                                 ...
                                 m_passwordField->rebuild()->password(!m_passwordVisible)->commence();
    ...
    // Error / info labels
    {
        ...
        m_errorLabel = CTextBuilder::begin()->text(std::string{""})->color([] { return CHyprColor{0.9F, 0.45F, 0.45F, 1.F}; })->commence();
